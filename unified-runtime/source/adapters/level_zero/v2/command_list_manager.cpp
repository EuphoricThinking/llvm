...
ur_result_t
ur_command_list_manager::appendUSMFill(void *pMem, size_t patternSize,
...
ur_result_t ur_command_list_manager::appendMemBufferRead(
    ...
    void *pDst, wait_list_view &waitListView,
    ur_event_handle_t phEvent) {
  ...
  return appendGenericCopyUnlocked(
      hBuffer, &dstHandle, blockingRead, offset, 0, size,
      waitListView, 
      phEvent, UR_COMMAND_MEM_BUFFER_READ);
}

ur_result_t
ur_command_list_manager::appendMemBufferWrite(ur_mem_handle_t hMem,
  ...
  return appendGenericCopyUnlocked(&srcHandle, hBuffer, blockingWrite, 0,
                                   offset, size, waitListView,
                                   phEvent, UR_COMMAND_MEM_BUFFER_WRITE);
}

ur_result_t
ur_command_list_manager::appendMemBufferCopy(ur_mem_handle_t hSrc,
                                             ur_mem_handle_t hDst,
  ...
  return appendGenericCopyUnlocked(hBufferSrc, hBufferDst, false, srcOffset,
                                   dstOffset, size, waitListView,
                                    phEvent, UR_COMMAND_MEM_BUFFER_COPY);
}
...
ur_command_list_manager::appendCommandBufferExp(ur_exp_command_buffer_handle_t
  ...
  UR_CALL(appendGenericCommandListsExp(1, &commandBufferCommandList, phEvent,
                                       waitListView, UR_COMMAND_ENQUEUE_COMMAND_BUFFER_EXP, 
...
ur_result_t ur_command_list_manager::appendDeviceGlobalVariableRead(
    ...
    size_t count, size_t offset, void *pDst,
    wait_list_view &waitListView, ur_event_handle_t phEvent) {
  TRACK_SCOPE_LATENCY(
...
ur_result_t ur_command_list_manager::bindlessImagesWaitExternalSemaphoreExp(
    ...
    uint64_t /*waitValue*/, wait_list_view & /* waitListView */, ur_event_handle_t /*phEvent*/) {
  return UR_RESULT_ERROR_UNSUPPORTED_FEATURE;
}
...
ur_result_t ur_command_list_manager::bindlessImagesSignalExternalSemaphoreExp(
    ...
    uint64_t /*signalValue*/, wait_list_view & /* waitListView */,ur_event_handle_t /*phEvent*/) {
  return UR_RESULT_ERROR_UNSUPPORTED_FEATURE;
}
